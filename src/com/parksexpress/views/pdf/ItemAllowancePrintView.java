...
public class ItemAllowancePrintView extends AbstractParksexpressPrintView {
	public ItemAllowancePrintView(){}
	
	private PdfPTable createTable(List<String> columns,
			List<AllowanceItem> data, String title) {
		final float[] widths = { .75f, .75f, 3.5f, 1.25f, 1.25f, 1f, 1f, 1f, 1f, 1f };
	...
	private void generateTableCells(final List<AllowanceItem> data, final PdfPTable table) {
		for (int i = 0; i < data.size(); i++) {
			final AllowanceItem item = data.get(i);
			...
			final Paragraph p0 = new Paragraph(item.getCheckDigitItemNumber(),
					baseFont);
			final Paragraph p1 = new Paragraph(item.getSize(), baseFont);
			final Paragraph p2 = new Paragraph(item.getDescription(), baseFont);
			final Paragraph p3 = new Paragraph(item.getCartonUPCNumber(), baseFont);
			final Paragraph p4 = new Paragraph(item.getRetailUPCNumber(), baseFont);
			final Paragraph p5 = new Paragraph("$" + format.format(item.getMarketCost()),
					baseFont);
			final Paragraph p6 = new Paragraph(
					format.format(item.getCostAllowance()), baseFont);
			final Paragraph p7 = new Paragraph("$" + format.format(item.getSpecialPrice()),
					baseFont);
			final Paragraph p8 = new Paragraph(item.getStartDate(), baseFont);
			final Paragraph p9 = new Paragraph(item.getExpirationDate(), baseFont);
	...
	public void renderData(Document document, Map map) throws Exception {
		...
		final PdfPTable table = this.createTable(columns, (List<AllowanceItem>) map
				.get("data"), "Item Allowances");
		document.add(table);
	}
}
