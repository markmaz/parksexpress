...
<div id="reportHeader" style="width: 830px; border: 1px #e5e5e5 solid; margin: 8px;  background-color: #f9f9f9;">
	<div style="font-size: large; color:#a04617; padding: 10px; float: left">Price Change Report</div>
	<script type="text/javascript" src="scripts/sorttable.js"></script>
	<script type="text/javascript" src="scripts/priceChangeReport/priceChangeReport.js"></script>
	<div style="text-align: right; padding: 10px; "><a href="#" onclick="printReport()">Print</a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="exportReport()">Export</a></div>
</div>

<div id="searchHeader" style="width: 830px; border: 1px #e5e5e5 solid; margin: 8px;  background-color: #f9f9f9;">
	<div>
		...
		<div id="searchCriteria" style="padding: 10px; ">
			<table>
				...
				<tr>
					...
					<td style="border: 0px;" colspan="4" align="right"><input type="button" value="Search" onclick="submit()"/><input id="clear" type="button" value="Clear" onclick="clearFields()"/></td>
				</tr>		
			</table>
		</div>
	</div>
...
<div id="zones" class="zones"> 
	<c:forEach var="zone" items="${zones}" varStatus="status">
		<c:choose>
			<c:when test="${status.index == 0}">
				<div id="${zone.number}" class="selectedTab" onclick="selectTab('${zone.number}')" onmouseover="rollOverTab('${zone.number}')" onmouseout="rollOverTab('${zone.number}')">${zone.name}</div>
			</c:when>
			<c:otherwise>
				<div id="${zone.number}" class="normalTab" onclick="selectTab('${zone.number}')" onmouseover="rollOverTab('${zone.number}')" onmouseout="rollOverTab('${zone.number}')">${zone.name}</div>	
			</c:otherwise>
		</c:choose>
	</c:forEach>
</div>
<div id="dataTab" class="dataSection">
	<div id="data" class="data" style="height: 400px">
