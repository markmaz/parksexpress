...
<%@ taglib uri="/fmt" prefix="fmt" %>
		<div class="contentPanelSRP" id="item" style="width: 680px;">
			<span style="margin: 5px"><strong>${item.description}</strong></span>
					...
					<tr>
						<c:choose>
							<c:when test="${isParksBook == true }">
								<td><span style="font-size: x-small">${item.checkDigitItemNumber} - ${item.description}</span></td>
							</c:when>
							<c:otherwise>
								<td><span style="font-size: x-small">
										<c:choose>
											<c:when test="${not empty item.futurePricing}">
											<c:choose>
												<c:when test="${item.futurePricing.fixed}">
													<a title="Effective Date: ${item.futurePricing.effectiveDate} Price: <fmt:formatNumber type="currency" currencySymbol="$"  maxFractionDigits="2" maxIntegerDigits="4" value="${item.futurePricing.price}"/>" style="font-size: x-small; color: #a04617" id="A_${item.number}" href="#" onclick="openModal('${item.number}', '${item.description}')">
												</c:when>
												<c:otherwise>
													<a title="Effective Date: ${item.futurePricing.effectiveDate} Percent: <fmt:formatNumber maxFractionDigits="2" maxIntegerDigits="4" value="${item.futurePricing.percent}"/>%" style="font-size: x-small; color: #a04617" id="A_${item.number}" href="#" onclick="openModal('${item.number}', '${item.description}')">												
												</c:otherwise>
											</c:choose>
										</c:when>
										<c:otherwise>
											<a style="font-size: x-small" id="A_${item.number}" href="#" onclick="openModal('${item.number}', '${item.description}')">
										</c:otherwise>
									</c:choose>
										${item.checkDigitItemNumber} - ${item.description}
									</a></span></td>
							</c:otherwise>
						</c:choose>
						<td><span style="width: 25px; font-size: x-small">${item.pack}</span></td>
						<td><span style="width: 25px; font-size: x-small">${item.size}</span></td>
						<td><span style="width: 25px; font-size: x-small">${item.cartonUPCNumber}</span></td>
						<td><span style="width: 25px; font-size: x-small">${item.retailUPCNumber}</span></td>
						<td><span style="font-size: x-small"><fmt:formatNumber type="currency" currencySymbol="$"  maxFractionDigits="2" maxIntegerDigits="4" value="${item.marketCost}"/></span></td>
						<td><span style="width: 25px; font-size: x-small"><fmt:formatNumber type="currency" currencySymbol="$"  maxFractionDigits="2" maxIntegerDigits="4" value="${item.pricing.price}"/></span></td>
						<td><span style="width: 25px; font-size: x-small"><fmt:formatNumber maxFractionDigits="2" maxIntegerDigits="4" value="${item.grossProfit}"/>%</span></td>
						<td>
							<c:choose>
								<c:when test="${item.pricing.fixed}">
									<input disabled="disabled" type="radio" name="${item.description}" value="F" checked>
									<input disabled="disabled" type="radio" name="${item.description}" value="P">
								</c:when>
								<c:otherwise>
									<input disabled="disabled" type="radio" name="${item.description}" value="F">
									<input disabled="disabled" type="radio" name="${item.description}" value="P" checked>							
								</c:otherwise>
							</c:choose>
						</td>
						<td align="center">
							<c:choose>
								<c:when test="${item.inOrderGuide}">
									<input type="checkbox" id="og_${item.number}" value="OG" checked onclick="updateOrderGuide('${item.number}', 'og_${item.number}')">
								</c:when>
								<c:otherwise>
									<input type="checkbox" id="og_${item.number}" value="NOG" onclick="updateOrderGuide('${item.number}', 'og_${item.number}')">
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
			...
			<div style="background-color: #fff">
				<table width="575px">
					...
					<tr>
						<c:choose>
							<c:when test="${isParksBook == true }">
								<td><span style="font-size: x-small">${family.familyCode} - ${family.description}</span></td>
							</c:when>
							<c:otherwise>
								<td><span style="font-size: x-small">
									<c:choose>
